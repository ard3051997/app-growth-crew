...
For a mobile subscription app, ground pricing decisions in real RevenueCat data rather than the SaaS-pricing-page framing this skill defaults to:

- `mcp__revenuecat__list_offerings`, `mcp__revenuecat__get_offering` — current tier/package structure
- `mcp__revenuecat__get_overview_metrics`, `mcp__revenuecat__get_charts` — conversion and revenue by tier, to identify which price point is actually underperforming
- `mcp__revenuecat__create_offering`, `mcp__revenuecat__update_offering`, `mcp__revenuecat__create_package` — ship a pricing/packaging change. **Mutating** — route through the approval gate in `src/app_manager/safety_policies.py`.
- Competitor pricing-page teardown (the `references/pricing-page-teardown.md` workflow) has no MCP-GC equivalent — there's no live competitor-pricing feed. Ask the user to paste competitor pricing screenshots/URLs for manual review, or cross-check `mcp__aso-keyword__analyze_listing_aso` for what a competitor app's store listing claims about its tiers.
