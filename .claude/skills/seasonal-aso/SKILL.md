...
Use MCP-GC's keyword tools to find volume on seasonal terms:

```
mcp__aso-keyword__get_keyword_volume(keywords=["christmas planner","holiday tracker"])
mcp__aso-keyword__search_keywords(term="christmas", country="us")
```

There's no free trending-keyword feed (see [mcp-gc.md](../aso-tools/integrations/mcp-gc.md) gaps) —
seed candidates from current news/culture instead and confirm real demand with
`get_keyword_volume` before committing.
