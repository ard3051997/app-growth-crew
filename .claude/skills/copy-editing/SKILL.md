...
This is a pure writing skill and needs no live data for most edits. When the copy being edited is an app's store listing text specifically:

- `mcp__app-store__get_app_details`, `mcp__play-store__get_listing` — pull the exact current published copy to edit, rather than working from a pasted/possibly-stale version
- `mcp__aso-keyword__score_auto_aso_metadata` — validate that an edit hasn't broken character limits or diluted target keywords (ASO-specific concerns copy-editing alone won't catch)

## Core Philosophy

**Check for product marketing context first:**
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename, in older setups; or MCP-GC's own app-marketing-context.md in the project root, if the ASO-tools fork's app-marketing-context skill has already been run), read it before editing. Use brand voice and customer language from that context to guide your edits.
