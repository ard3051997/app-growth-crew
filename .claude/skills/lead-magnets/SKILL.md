...
This skill was written for email-capture-driven B2B/SaaS growth; it applies most directly to a companion website or pre-launch waitlist, not the app itself. Where it does apply to an MCP-GC app:

- `mcp__fcm-push__send_push_to_condition` — re-engage users who opted in to an in-app "unlock this free resource/template" style offer, instead of an email nurture sequence
- `mcp__aso-keyword__search_keywords`, `mcp__aso-keyword__get_keyword_volume` — validate lead-magnet topic demand against real App Store search terms rather than guessing
- Actual email capture and delivery (the mechanism this skill assumes throughout) has no MCP-GC tool — see the gap noted in the `emails` skill; bring your own ESP for the capture form and delivery sequence.
