...
Mobile install attribution is a different problem from web attribution — there's no cookie, and iOS 14.5+ collapses most of it behind SKAdNetwork. Ground Pillar A (interpretation) with real numbers, and treat Pillar B as already largely solved:

- `mcp__analytics__get_user_acquisition` — GA4/Firebase's acquisition-channel report, the closest MCP-GC equivalent to "your dashboards disagree" reconciliation
- `mcp__funnel-engine__run_funnel_analysis`, `mcp__funnel-engine__get_historical_listing_conversion` — first-party, exact conversion data for MCP-GC's own apps (no cross-tool disagreement, since it's one pipeline)
- `mcp__revenuecat__get_overview_metrics` — the "which channel drives revenue" question, for subscription revenue specifically
- For SKAdNetwork/AdAttributionKit, MMP setup (AppsFlyer/Adjust/Singular), deep links, and conversion-value schemas — that's a distinct, deeper topic than this skill covers. See the ASO-tools fork's `attribution-setup` skill instead; it's the mobile-specific specialist and should be preferred over this skill's web-oriented Pillar B instructions for anything install-attribution related.

Most requests start with (A). Reach for (B) only when they control the surface and want to build.

Product context: check for `.agents/product-marketing.md` (or MCP-GC's own `app-marketing-context.md` in the project root, if the ASO-tools fork's `app-marketing-context` skill has already been run) and read it if present — business type, sales cycle, and primary conversion drive almost every recommendation here.
