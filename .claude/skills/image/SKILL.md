...
- `mcp__image-gen__generate_image` — MCP-GC's bound image-generation tool; use it for the AI-generation portion of this skill (blog heroes, social graphics, mockups, banners) instead of a third-party model API
- For App Store/Play Store screenshot and icon generation specifically, prefer the ASO-tools fork's `screenshot-optimization` and `app-icon-optimization` skills — they apply this same tool with store-specific size/format constraints
- Design-tool integrations (Canva, Figma) referenced by this skill have no MCP-GC binding here — if Canva/Figma MCP access is configured in the user's environment, use it directly; otherwise treat those steps as manual.
- Image compression/WebP conversion has no dedicated MCP-GC tool — handle with standard image tooling outside MCP-GC.
