...
Relevant when an MCP-GC app has a companion marketing/support site (press kit, help docs, landing page) — the app store listings themselves are covered by ASO tooling, not this skill.

- `mcp__openseo__get_search_console_performance` — see whether the site is already appearing in Google's own AI Overviews reporting
- `mcp__openseo__get_serp_results`, `mcp__openseo__find_serp_competitors` — check current SERP composition (AI Overview presence, featured snippets) for target queries
- `mcp__openseo__research_keywords` — the question/query research this skill's citation strategy depends on
- `mcp__openseo__run_site_audit` — technical crawlability baseline (a prerequisite for AI-crawler access, same as regular SEO)
