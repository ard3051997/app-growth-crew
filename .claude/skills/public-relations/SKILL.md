...
- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — mine real user reviews for quotable pain points, delight moments, and "human" story evidence (the pitchable angles this skill asks for)
- `mcp__app-store__get_app_details`, `mcp__play-store__get_listing` — pull current ratings/download-adjacent facts to back a pitch with real numbers
- `mcp__revenuecat__get_overview_metrics` — real growth/revenue milestones ("we crossed X subscribers") are strong PR hooks — pull the real number instead of estimating
- Journalist database access, HARO/Qwoted queries, and media-list building have no MCP-GC tool — that's outside the current tool surface; use `WebSearch`/`WebFetch` or bring your own PR CRM.
- The ASO-tools fork's `press-and-pr` skill is the app-store-specific specialist (App Store editorial, featuring) — use it alongside this skill, which covers general earned-media/journalist-pitching mechanics this library doesn't otherwise have.
