...
**No free market-wide movement feed exists** (this required Appeeky's paid market
endpoints upstream — see [mcp-gc.md](../aso-tools/integrations/mcp-gc.md) gaps).
Reconstruct movement yourself instead of one live call:

1. **`mcp__aso-keyword__get_category_top_apps`** — pull current chart standings for the category/country
2. Compare against a **prior snapshot** you saved last time this skill ran (ask the
   user for it, or check a scratch/notes file if one exists) to derive gainers,
   losers, new entries, and drop-outs
3. **`mcp__app-store__get_app_details`** / **`mcp__play-store__get_listing`** — deep dive on specific apps showing movement
4. If no prior snapshot exists, say so plainly, save this pull as the new baseline, and offer to re-run in a week

Be upfront that this is a manual-diff approximation, not a live market feed.
