...
**Check for product marketing context first:** if `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md`; or MCP-GC's own `app-marketing-context.md` in the project root, if the ASO-tools fork's `app-marketing-context` skill has already been run), read it before asking questions. Use that context and only ask for what's missing.

## MCP-GC Tools

Loops that orchestrate the other skills in this library inherit whatever MCP-GC tools those skills use (see each skill's own "MCP-GC Tools" section and `.claude/skills/marketing-tools/REGISTRY.md`). The loop-scheduling mechanism itself has two options in this project:

- MCP-GC's own `src/run_autonomous_loop.py` already runs a scheduled loop over the domain specialists in `src/app_manager/` — a marketing loop defined here can be expressed as an additional scheduled task in that same system rather than a separate scheduler.
- Claude Code's own `/loop` skill (if available in the environment) can run a prompt/slash-command on a recurring interval for lighter-weight, ad hoc loops that don't need the full experiment/rollback machinery.
