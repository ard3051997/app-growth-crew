...
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md`; or MCP-GC's own `app-marketing-context.md` in the project root, if the ASO-tools fork's `app-marketing-context` skill has already been run), read it before asking questions.

## MCP-GC Tools

This skill is persona simulation over the user's existing situation — it needs no live MCP-GC data of its own, but the *inputs* the council debates should be real: pull `mcp__revenuecat__get_overview_metrics`, `mcp__analytics__get_retention`, or `mcp__app-store__get_reviews` (as relevant to the question) before convening the council, so the advisors are reacting to actual numbers rather than assumptions.
