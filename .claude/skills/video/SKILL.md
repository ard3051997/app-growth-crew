...
- `mcp__mobile-mcp__mobile_start_screen_recording`, `mcp__mobile-mcp__mobile_stop_screen_recording` — capture real in-app screen footage on a device/simulator, the raw material for product-demo and App Preview-style videos (this is the one piece of the video pipeline MCP-GC can supply directly)
- `mcp__image-gen__generate_image` — generate stills/frames/thumbnails to storyboard a video before producing it
- AI video generation (Runway, Veo, Sora, Kling, HeyGen avatars, Remotion/Hyperframes programmatic pipelines) and actual editing/rendering have no MCP-GC tool — this is squarely outside the current tool surface. Bring your own video-generation and editing tools; MCP-GC can only supply the source screen capture and generated stills that feed into them.
- For App Store Preview / Play Store promo video strategy specifically (specs, poster frames, A/B testing), see the ASO-tools fork's `app-preview-video` skill instead — this skill is the general production/tooling layer, that one is the store-listing strategy layer.
