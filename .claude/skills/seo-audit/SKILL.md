...
Relevant for an app's companion marketing/support website, not the App Store/Play Store listing itself (for store-listing "why am I not ranking," use the ASO-tools fork's `aso-audit` instead — that's a different ranking system entirely).

- `mcp__openseo__run_site_audit`, `mcp__openseo__get_audit_status`, `mcp__openseo__get_audit_issues`, `mcp__openseo__get_audit_pages` — the technical crawl/audit this skill is built around
- `mcp__openseo__get_search_console_performance`, `mcp__openseo__inspect_urls` — indexing status and real Search Console data instead of guessing
- `mcp__openseo__get_domain_overview`, `mcp__openseo__get_ranked_keywords` — current organic visibility baseline
- `mcp__openseo__get_backlinks_overview` — backlink-profile health, if the audit needs it
- This server (`openseo`) is being added elsewhere in this branch and may not be live in every worktree yet.
