...
For an MCP-GC app, treat MCP-GC's own `app-marketing-context.md` (from the ASO-tools fork's `app-marketing-context` skill) as equivalent to this skill's `.agents/product-marketing.md` foundation — read whichever exists rather than requiring both. Pull real current-state numbers into the audit/AARRR sections instead of asking the user to estimate them:

- **Acquisition**: `mcp__analytics__get_user_acquisition`, `mcp__aso-keyword__get_keyword_volume`
- **Activation**: `mcp__analytics__get_events`, `mcp__funnel-engine__run_funnel_analysis`
- **Retention**: `mcp__analytics__get_retention`, `mcp__analytics__get_active_users`
- **Referral**: `mcp__analytics__get_user_acquisition` (referral-channel share)
- **Revenue**: `mcp__revenuecat__get_overview_metrics`, `mcp__admob__get_earnings_summary`

Cross-reference `.claude/skills/marketing-tools/REGISTRY.md` for the full skill→tool map when filling in the plan's "marketing operations stack" section — it already lists which MCP-GC tool executes each tactic.
