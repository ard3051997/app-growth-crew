...
## MCP-GC Tools — mostly out of scope for an app-only portfolio

This skill assumes a marketing website that can host an interactive calculator/tool for SEO/lead-gen. MCP-GC has no website-building or web-hosting tool — building and hosting a standalone free web tool is outside the current tool surface; bring your own site infrastructure if the user wants to pursue this.

Where it's still useful: validating demand for a proposed tool topic before building it —

- `mcp__openseo__research_keywords`, `mcp__openseo__get_search_opportunities` — real search-volume validation for the calculator/tool topic (server being added elsewhere in this branch)
- If the "free tool" idea is actually a free *in-app* feature/widget rather than a web tool, this becomes a product-marketing question — see `product-marketing` and `mcp__analytics__get_events` to measure adoption once shipped.
