...
- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — MCP-GC's primary voice-of-customer source; this is the direct mobile equivalent of this skill's "Mode 2: Mine Existing Signal" (Reddit/G2/forum mining), except it's first-party and exact
- `mcp__analytics__get_user_demographics`, `mcp__analytics__get_events`, `mcp__analytics__get_screen_views` — behavioral signal to cross-check what users say against what they actually do
- `mcp__funnel-engine__run_funnel_analysis` — where users are actually dropping off, to ground "what are customers struggling with"
- G2/Reddit/forum mining outside the app-store review surface has no MCP-GC tool — use `WebSearch`/`WebFetch` for that, same as the upstream skill assumes.
- Primary research (running interviews/surveys — Mode 3) has no MCP-GC tool either; that stays manual.

## Before Starting

**Check for product marketing context first:**
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename, in older setups; or MCP-GC's own app-marketing-context.md in the project root, if the ASO-tools fork's app-marketing-context skill has already been run), read it before asking questions. Use that context to skip questions already answered.
