...
## MCP-GC Tools — no MCP-GC tool for this one

SMS/MMS sending requires a real carrier-connected provider (Twilio, Postscript, Attentive, etc. — exactly what this skill already recommends). MCP-GC has no SMS provider integration; this is genuinely outside the current tool surface, not a gap we can paper over. Bring your own Twilio/Postscript/Attentive account and credentials to execute anything this skill plans.

For the mobile-app equivalent of a re-engagement/lifecycle channel that MCP-GC *does* have a first-party tool for, use `mcp__fcm-push__send_push_notification` / `send_push_to_condition` / `send_push_to_token` (push notifications) instead — it covers most of the same win-back/abandoned-action/promotional use cases this skill describes for SMS, without needing a third-party account.
