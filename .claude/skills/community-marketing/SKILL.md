...
MCP-GC has no direct Discord/Slack/Reddit-management binding — community platform operations are out of scope for the current tool surface; bring your own bot/credentials for that layer. Where this skill overlaps with app data:

- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — the closest thing MCP-GC has to "listening" to the community: app reviews are often where the most engaged (and most vocal) users already are
- `mcp__app-store__reply_to_review`, `mcp__play-store__reply_to_review` — publicly engaging reviewers is a lightweight community-building action available today. **Mutating** — route through the approval gate in `src/app_manager/safety_policies.py`.
- `mcp__fcm-push__send_push_to_condition` — invite an in-app user segment into a community (e.g., "power users" cohort) once you know who they are
- For identifying candidate community members/advocates by engagement, use `mcp__analytics__get_active_users` and `mcp__analytics__get_retention` to find your most-retained cohort first.

## Before You Start

**Check for product marketing context first:**
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename, in older setups; or MCP-GC's own app-marketing-context.md in the project root, if the ASO-tools fork's app-marketing-context skill has already been run), read it before asking questions. Use that context and only ask for information not already covered.
