...
- `mcp__analytics__get_user_acquisition` — measure referral-driven installs against other channels
- `mcp__revenuecat__get_overview_metrics`, `mcp__revenuecat__grant_entitlement` — track referral-reward redemption and grant referral incentives (free months, unlocked entitlements) as RevenueCat entitlement grants. **`grant_entitlement` is mutating** — route through the approval gate in `src/app_manager/safety_policies.py`.
- `mcp__fcm-push__send_push_to_condition`, `mcp__fcm-push__send_push_notification` — nudge existing users to invite friends, or notify a user their referral converted
- Deep-link/invite-link infrastructure (attribution for who invited whom) is covered by the ASO-tools fork's `attribution-setup` skill, not this one — use both together.
- The ASO-tools fork also has a `referral-program` skill scoped specifically to in-app invite mechanics; this skill is the broader affiliate/ambassador-program version (useful when payouts or B2B partners are involved). Prefer `referral-program` for pure in-app viral loops.
