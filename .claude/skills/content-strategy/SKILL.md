...
Relevant for a companion website/blog or App Store "What's New" / in-app content, not the store listing itself (metadata content strategy is covered by the ASO-tools fork's `metadata-optimization`).

- `mcp__openseo__research_keywords`, `mcp__openseo__get_search_opportunities` — ground topic selection in real search demand instead of guessing (server being added elsewhere in this branch)
- `mcp__aso-keyword__search_keywords`, `mcp__aso-keyword__get_keyword_volume` — cross-check topic ideas against App Store search demand, if the content is meant to reinforce ASO
- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — mine reviews for content topics that map to real user questions/pain points
