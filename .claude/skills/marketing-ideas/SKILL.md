...
This is a brainstorming/selection skill, not a data-pull skill — no tool is required to use it. Once an idea is selected, hand off execution to the relevant channel skill in this library (which carries its own MCP-GC tool bindings) or to `.claude/skills/marketing-tools/REGISTRY.md` for the full skill→tool map. To ground *which* ideas are worth trying, `mcp__funnel-engine__get_funnel_health_score` and `mcp__analytics__get_retention` can identify the weakest stage of the funnel first, so idea selection targets a real bottleneck rather than a random tactic.
