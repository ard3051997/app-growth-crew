...
- `mcp__image-gen__generate_image` — generate static ad creative concepts, backgrounds, and hero imagery for the formats this skill covers (static ads, greenscreen/UGC-style stills, iMessage-format mockups)
- `mcp__meta-ads__*` — push finished creative into a Meta campaign and pull real performance data to close the iteration loop (`references/creative-review-page.md`'s "iterate based on real performance data" step). Being added elsewhere in this branch; not yet live in every worktree.
- `mcp__mobile-mcp__mobile_take_screenshot`, `mcp__mobile-mcp__mobile_start_screen_recording` — capture real in-app footage as raw material for UGC-style or motion/video ad creative, instead of only mocking it up
- Actual video editing/rendering has no MCP-GC tool — bring your own (CapCut, Premiere, etc.) for the motion/video-ad formats this skill describes; MCP-GC can only supply the source screen capture and generated stills.
