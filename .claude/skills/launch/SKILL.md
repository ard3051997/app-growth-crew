...
- `mcp__aso-keyword__get_category_top_apps`, `mcp__aso-keyword__search_keywords` — category/keyword landscape research to time and position a launch
- `mcp__fcm-push__send_push_to_condition` — notify an existing user base about a major feature launch or re-launch
- `mcp__analytics__get_realtime_report`, `mcp__analytics__get_user_acquisition` — launch-day monitoring
- `mcp__revenuecat__get_overview_metrics` — track conversion impact of a launch in real time
- For a full **new app** launch specifically (App Store submission timeline, pre-launch checklist, Apple editorial pitch), the ASO-tools fork's `app-launch` skill is the dedicated specialist and already carries the store-specific tool bindings — prefer it over this skill for that case. This skill is the better fit for feature launches, Product Hunt-style announcements, and general GTM sequencing.
