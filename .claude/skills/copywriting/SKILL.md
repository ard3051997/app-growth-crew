...
This is a pure writing skill and needs no live data for a companion-site landing page. For App Store/Play Store listing copy specifically, prefer the ASO-tools fork's `metadata-optimization` skill instead — it's already bound to `mcp__aso-keyword__generate_optimized_listing` / `score_auto_aso_metadata` and enforces store character limits this skill doesn't know about. Use:

- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — pull real customer language (this skill's "voice of customer" input) instead of inventing it
- `mcp__revenuecat__get_overview_metrics` — real numbers/social proof to back a claim in the copy
