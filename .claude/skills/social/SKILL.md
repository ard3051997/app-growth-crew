...
MCP-GC has no social-platform posting/scheduling/listening API bound (no Buffer/Hootsuite/native-platform-API equivalent) — content creation and strategy below is fully usable, but publishing, scheduling, and social listening are outside the current tool surface; bring your own scheduler/credentials for those steps.

- `mcp__image-gen__generate_image` — generate the graphics/carousel slides this skill calls for
- `mcp__mobile-mcp__mobile_start_screen_recording`, `mcp__mobile-mcp__mobile_take_screenshot` — source real in-app footage/screens for short-form video and "show don't tell" posts
- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — mine reviews for quotable social-proof content and repurposable user language
- `mcp__revenuecat__get_overview_metrics` — real milestone numbers ("10,000 subscribers") for milestone/social-proof posts
