...
- `mcp__mobile-mcp__mobile_take_screenshot`, `mcp__mobile-mcp__mobile_list_elements_on_screen` — walk the real first-run flow on a device/simulator instead of describing it from memory; essential for spotting friction in permission prompts, empty states, and step count
- `mcp__analytics__get_retention` — Day 1/7/30 retention, the hard measure of whether onboarding actually worked
- `mcp__funnel-engine__run_funnel_analysis` — step-by-step onboarding funnel drop-off
- `mcp__analytics__get_events` — confirm the "aha moment" event is actually firing and see how many users reach it
- For a specialist already tuned to this exact problem for MCP-GC apps, see the ASO-tools fork's `onboarding-optimization` skill — the two should agree on tooling and can be used together (this skill for the activation-model theory, that one for the App Store-context specifics).
