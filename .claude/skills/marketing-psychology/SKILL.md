...
Conceptual skill — no live data is required to apply a mental model. When testing whether a model (anchoring, social proof, scarcity) actually moves behavior for an MCP-GC app, ground it in real results rather than theory alone: `mcp__revenuecat__get_overview_metrics` and `mcp__revenuecat__create_offering`/`update_offering` to implement and measure a psychology-informed paywall/pricing change (mutating — route through the approval gate in `src/app_manager/safety_policies.py`), or `mcp__funnel-engine__run_funnel_analysis` to see if a framing change actually moved conversion.

## How to Use This Skill

**Check for product marketing context first:**
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename, in older setups; or MCP-GC's own app-marketing-context.md in the project root, if the ASO-tools fork's app-marketing-context skill has already been run), read it before applying mental models. Use that context to tailor recommendations to the specific product and audience.
