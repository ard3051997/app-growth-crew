...
**MCP-GC note:** also check for `app-marketing-context.md` (project root or `.claude/`) — the output of the ASO-tools fork's `app-marketing-context` skill. It covers the same ground (product, audience, positioning) for an app specifically, with app-store-flavored sections (category, App Store/Play Store context) this skill's generic template doesn't have. If it exists, treat it as the source of truth rather than creating a second, competing context doc — offer to either (a) use `app-marketing-context.md` as-is for every other skill in this library, or (b) generate `.agents/product-marketing.md` as a derived summary of it for skills that specifically look for that filename. Don't maintain two independently-edited context docs for the same app.
