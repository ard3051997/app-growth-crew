...
This skill is mostly conceptual (bonus stacking, guarantees, framing) and was written for services/courses/coaching — most of it applies unchanged. Where the offer is actually implemented as an in-app purchase:

- `mcp__revenuecat__list_offerings`, `mcp__revenuecat__create_offering`, `mcp__revenuecat__create_package` — turn the constructed offer (bonus stack, trial length, guarantee-backed pricing) into a real RevenueCat offering once designed
- `mcp__revenuecat__get_overview_metrics` — measure whether the new offer actually converts better than the old one
- There is no MCP-GC equivalent for payment-plan/invoicing infrastructure (Stripe payment links, etc.) — that part of this skill is out of scope for an app-only portfolio; bring your own tool for high-ticket service/coaching offers sold outside the app.
