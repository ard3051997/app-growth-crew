...
Fetch via MCP-GC's own tools (see [REGISTRY.md](../aso-tools/REGISTRY.md)):
- App metadata (title, subtitle, description, screenshots, ratings) — `mcp__app-store__get_app_details` / `mcp__play-store__get_listing`
- Current keyword rankings — `mcp__aso-keyword__get_app_keywords`
- Competitor data (top 3-5 in same category) — `mcp__aso-keyword__get_competitor_keywords`, `get_category_top_apps`
- Category chart position — `mcp__aso-keyword__get_category_top_apps`
- Review sentiment — `mcp__app-store__get_reviews` / `mcp__play-store__get_reviews`

If a source isn't configured for this app in `config/apps.json`, say so and ask the user to provide their current metadata instead.
