...
MCP-GC apps run user acquisition (UA), not B2B lead-gen — treat "leads" in this skill's generic instructions as "installs" / "trial starts", and use:

- `mcp__meta-ads__*` — campaign creation, audience targeting, budget/bid management on Meta (Facebook/Instagram). This server is being added elsewhere in this branch; not yet live in every worktree, but this is MCP-GC's bound tool for Meta paid UA. Google Ads / LinkedIn / Twitter-X campaign management has no MCP-GC tool — bring your own platform access for those.
- `mcp__aso-keyword__get_keyword_volume`, `mcp__aso-keyword__search_keywords` — keyword-driven audience/creative targeting research (what people actually search for in the store, useful for ad angle and Apple Search Ads keyword overlap)
- `mcp__analytics__get_user_acquisition` — GA4 acquisition-channel performance, to check UA spend is actually converting to activated users, not just installs
- `mcp__funnel-engine__run_funnel_analysis`, `mcp__funnel-engine__get_benchmark_comparison` — attribute install→activation→purchase drop-off back to a specific campaign/creative
- For Apple Search Ads specifically (a distinct paid-UA channel from Meta), see the ASO-tools fork's `apple-search-ads` skill instead — that's the right specialist for App Store search placement.

## Before Starting

**Check for product marketing context first:**
If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or the legacy `product-marketing-context.md` filename, in older setups; or MCP-GC's own app-marketing-context.md in the project root, if the ASO-tools fork's app-marketing-context skill has already been run), read it before asking questions. Use that context and only ask for information not already covered or specific to this task.
