...
**Most of the signals this briefing wants (chart movers, trending keywords, featured
apps, new releases, #1 entrants, download benchmarks) require a paid market-intel
feed this project doesn't have** — see [mcp-gc.md](../aso-tools/integrations/mcp-gc.md)
gaps. Don't fabricate these; instead assemble the briefing from what's actually
available:

1. **`mcp__aso-keyword__get_category_top_apps`** — current chart standings for the user's category (diff against a prior saved snapshot for movement, same method as `market-movers`)
2. **`mcp__aso-keyword__search_keywords`** + **`get_keyword_volume`** — check specific seasonal/news-driven terms for a real volume signal, rather than a trending feed
3. **`mcp__app-store__get_app_details`** / **`mcp__play-store__get_listing`** — spot-check specific apps mentioned
4. WebSearch for App Store editorial/press coverage as a substitute for a "featured apps" feed

State plainly which sections of the briefing are unavailable this run rather than omitting them silently.
