...
- `mcp__app-store__get_reviews`, `mcp__play-store__get_reviews` — a free source of organic advocates: mine 5-star reviews for users who already sound like enthusiastic creators/ambassadors worth approaching
- `mcp__analytics__get_user_acquisition` — measure whether a creator campaign actually moved installs, post-campaign
- `mcp__revenuecat__get_overview_metrics` — tie a creator/ambassador cohort's promo code or link to real subscription revenue, if tracked
- Creator discovery/vetting databases, payment/contract handling, and disclosure-compliance tracking have no MCP-GC tool — that layer is outside the current tool surface; bring your own creator-marketplace or spreadsheet workflow.
- The ASO-tools fork's `creator-ugc-marketing` skill covers the same ground with an app-specific lens (TikTok/Reels creators, UGC ad creative). The two overlap significantly — prefer `creator-ugc-marketing` for pure mobile-app UGC/creator work, and this skill for structured paid-partnership/ambassador-program mechanics (deal structuring, FTC disclosure, ROI measurement) it covers in more depth.

## Before Starting

**Check for product marketing context first.** If `.agents/product-marketing.md` exists (or `.claude/product-marketing.md`, or legacy `product-marketing-context.md`; or MCP-GC's own `app-marketing-context.md` in the project root, if the ASO-tools fork's `app-marketing-context` skill has already been run), read it before asking questions — the ICP, positioning, and offer anchor every partner-fit decision. Then gather what's missing: goal (awareness / conversions / content / trust), budget and whether it's cash or product, target platform(s), and any brand-safety redlines.
