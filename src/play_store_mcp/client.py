...
from mcp_gc_shared.write_guard import assert_writes_allowed
...
class PlayStoreClient:
    ...
    def list_voided_purchases(
        ...
        try:
            result = (
                service.purchases()
                .voidedpurchases()
                .list(packageName=package_name, maxResults=max_results, type=1)
                .execute()
            )
    ...
    def update_listing(
        ...
        Returns:
            Update result.
        """
        assert_writes_allowed("Play Store listing update")
    ...
    def update_listing_images(
        ...
        """Upload storefront screenshots/icons for a specific language.

        Args:
            package_name: App package name.
            language: Language code (e.g. en-US).
            image_type: Type of image ('icon', 'featureGraphic', 'phoneScreenshots', etc.).
            image_bytes_base64: Base64-encoded image bytes.
        """
        import base64
        import io
        ...
        try:
            ...
            self._commit_edit(package_name, edit_id)
            return {"success": True, "image_type": image_type, "language": language}
        except HttpError as e:
            self._logger.exception("Failed to upload listing image", error=str(e))
            self._delete_edit(package_name, edit_id)
            return {"success": False, "error": f"Failed to upload image: {e.reason}"}
        except Exception as e:
            self._logger.exception("Failed to upload listing image", error=str(e))
            self._delete_edit(package_name, edit_id)
            return {"success": False, "error": str(e)}

    def create_store_listing_experiment(
        self,
        package_name: str,
        experiment_config: dict[str, Any],
    ) -> dict[str, Any]:
        """Create a native Play Console store listing experiment.

        Args:
            package_name: App package name.
            experiment_config: Experiment configuration dictionary.
        """
        self._logger.info("Creating store listing experiment", package_name=package_name)
        service = self._get_service()
        edit_id = self._create_edit(package_name)

        try:
            if hasattr(service.edits(), "storelistingexperiments"):
                result = (
                    service.edits()
                    .storelistingexperiments()
                    .create(packageName=package_name, editId=edit_id, body=experiment_config)
                    .execute()
                )
                self._commit_edit(package_name, edit_id)
                ...
                self._logger.warning(
                    "storelistingexperiments API not available on edits service. Mocking success."
                )
                self._delete_edit(package_name, edit_id)
                return {
                    "success": True,
                    "mock": True,
                    "message": "API not supported by current google-api-client version, mocked success.",
                }
        except HttpError as e:
            self._logger.exception("Failed to create store listing experiment", error=str(e))
            self._delete_edit(package_name, edit_id)
            return {"success": False, "error": f"Failed to create experiment: {e.reason}"}
        except Exception as e:
            self._logger.exception("Failed to create store listing experiment", error=str(e))
            self._delete_edit(package_name, edit_id)
            return {"success": False, "error": str(e)}
